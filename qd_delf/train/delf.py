...
def __load_weights_from__(module_dict, load_dict, modulenames, loadnames=None):
    for i, modulename in enumerate(modulenames):
        ...
        module.load_state_dict(load_dict[loadname])
            ...
            if self.stage in ['keypoint']:
                ...
                for key in load_dict["state_dict"]:
                    if key.startswith("module."):
                        ...
                        m_key = key
                    if all([not m_key.startswith(ex) for ex in exclude]):
    ...
    def __register_module__(self, modulename, module):
        if isinstance(module, list) or isinstance(module, tuple):
            if isinstance(module[0], tuple):
                assert(all([len(m) == 2 for m in module]))
                module = nn.Sequential(collections.OrderedDict(module))
            else:
