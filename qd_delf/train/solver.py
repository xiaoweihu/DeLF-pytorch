...
def __set_bn_eval__(m):
    classname = m.__class__.__name__
    if classname.find('BatchNorm') != -1:
        m.eval()
        m.track_running_status = False

class MultiFixedScheduler(optim.lr_scheduler.MultiStepLR):
    ...
    def step(self, iterations=None):
        ...
        for param_group in self.optimizer.param_groups:
            param_group['lr'] = param_group.get('lr_mult', 1) * new_lr

class Solver(object):
    def __init__(self, config, model, max_iters=None, num_iters_per_epoch=None):
        ...
        if config.lr_policy == "STEP":
            self.lr_scheduler = optim.lr_scheduler.StepLR(
                self.optimizer,
                step_size=config.lr_stepsize * num_iters_per_epoch,
                gamma=config.lr_gamma)
        elif config.lr_policy == "MULTIFIX":
            stages = map(lambda x:int(x*max_iters/70.0), [1, 50, 60, 70])
            stage_lrs = [0.0001, 0.01, 0.0001, 0.00001]
            self.lr_scheduler = MultiFixedScheduler(self.optimizer, stages, stage_lrs)
        else:
            raise ValueError("unknown lr policy: {}".format(config.lr_policy))
        ...
        with open(os.path.join(self.prefix, config.stage, 'log/config.yaml'), 'w') as fp:
            yaml.dump(self.state, fp, default_flow_style=False)
    ...
    def __solve__(self, mode, epoch, dataloader):
        ...
        for batch_idx, (inputs, labels) in enumerate(dataloader):
            ...
            if mode in ['train']:
                ...
                if self.config.bn_eval_mode:
                    for module in self.model.children():
                        module.apply(__set_bn_eval__)
            ...
            prec_losses.update(__to_tensor__(loss).item(), batch_size)
